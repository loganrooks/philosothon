...
const mockQuestions: Question[] = [
  ...
  },
  {
    ...
    type: 'textarea', // Changed from 'ranking-numbered'
    required: false,
...
const enterInput = async (text: string) => {
  ...
  // Wait again for effects of the submission
  await act(async () => { await new Promise(resolve => setTimeout(resolve, 50)); });
};

// Helper to get the displayed output history
const getOutputHistory = () => {
    const outputDiv = screen.getByTestId('terminal-output'); // Assuming a data-testid on the output container
...
describe('RegistrationForm (Terminal UI V3 - Red Phase)', () => {
    ...
    it('should render initial V3 boot sequence and guest prompt', async () => {
        ...
        expect(await screen.findByText(/\[guest@philosothon]\$/)).toBeInTheDocument();
        expect(screen.getByText(/Type 'help' for available commands./i)).toBeInTheDocument();
        // Check for V3 anonymous commands in help hint
        expect(screen.getByText(/register/i)).toBeInTheDocument();
        expect(screen.getByText(/sign-in/i)).toBeInTheDocument();
        expect(screen.getByText(/about/i)).toBeInTheDocument();
    });

    it('should show V3 intro and prompt for Name (Q1) on "register new"', async () => {
        render(<RegistrationForm />);
        expect(await screen.findByText(/\[guest@philosothon]\$/)).toBeInTheDocument();
        await enterInput('register new');
        // Check for V3 intro text
        expect(await screen.findByText(/Welcome to the Philosothon registration form!/i)).toBeInTheDocument();
        expect(await screen.findByText(/April 26-27, 2025/i)).toBeInTheDocument();
        // Check for first V3 question prompt (Name - Q1)
        expect(await screen.findByText(/\[registration]>/)).toBeInTheDocument(); // V3 prompt
        const nameQuestion = actualV3Questions.find(q => q.id === 'fullName');
        expect(nameQuestion).toBeDefined();
        expect(screen.getByText(nameQuestion!.label)).toBeInTheDocument(); // Q1 Label
    });

     it('should proceed through early auth flow (Name -> Email -> PW -> Confirm -> signUpUser -> Q3)', async () => {
        ...
        // Q1: Name
        expect(await screen.findByText(/Full Name/i)).toBeInTheDocument();
        await enterInput('Test User V3');

        // Q2: Email
        expect(await screen.findByText(/University Email Address/i)).toBeInTheDocument();
        await enterInput('v3@test.com');

        // Q2.1: Password
        expect(await screen.findByText(/^Password:/i)).toBeInTheDocument();
        await enterInput('passwordV3');

        // Q2.2: Confirm Password
        expect(await screen.findByText(/Confirm Password:/i)).toBeInTheDocument();
        await enterInput('passwordV3'); // Matching password

        // Check signUpUser was called
        await waitFor(() => {
            expect(mockSignUpUser).toHaveBeenCalledWith({ email: 'v3@test.com', password: 'passwordV3' });
        });

        // Check it proceeds to Q3 (Pronouns - based on V3 schema order 5)
    ...
    // Add more V3 failing tests here for commands, modes, input types etc.
    // e.g., test 'sign-in', test 'back', test multi-select input
